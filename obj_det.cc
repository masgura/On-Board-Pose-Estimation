...
torch::Tensor non_max_supperession(torch::Tensor& prediction, float conf_thres = 0.5, float iou_thres = 0.45, int max_det = 300) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // If a key is pressed or the window is closed, close the window
        if (key != -1) {
